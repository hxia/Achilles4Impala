...
from
	(select person_id, count(OBSERVATION_period_start_date) as num_periods from @cdm_database_schema.OBSERVATION_PERIOD group by PERSON_ID) op1
...
from
	@cdm_database_schema.PERSON p1
	inner join (select person_id, count(payer_plan_period_start_date) as num_periods from @cdm_database_schema.payer_plan_period group by PERSON_ID) ppp1
	on p1.person_id = ppp1.person_id
