...
select 202 as analysis_id,   
   cast(vo1.visit_concept_id as string) as stratum_1,
   cast(year(cast(visit_start_date as timestamp))*100 + month(cast(visit_start_date as timestamp)) as string) as stratum_2, 
   count(distinct PERSON_ID) as count_value
from visit_occurrence vo1
group by vo1.visit_concept_id, 
	year(cast(visit_start_date as timestamp))*100 + month(cast(visit_start_date as timestamp))
;
...
with rawData as
(
    select person_id, count(distinct visit_concept_id) as count_value
...
select 204 as analysis_id,   
   cast(vo1.visit_concept_id as string) as stratum_1,
   cast(year(cast(vo1.visit_start_date as timestamp)) as string) as stratum_2,
   cast(p1.gender_concept_id as string) as stratum_3,
   cast(floor((year(cast(vo1.visit_start_date as timestamp)) - p1.year_of_birth)/10) as string) as stratum_4, 
...
group by vo1.visit_concept_id, 
	year(cast(vo1.visit_start_date as timestamp)), 
	p1.gender_concept_id, 
	floor((year(cast(vo1.visit_start_date as timestamp)) - p1.year_of_birth)/10)
;
...
(
  select vo1.visit_concept_id as stratum1_id, 
   ...
   from person p1
	inner join
	  (
		  select person_id, visit_concept_id, min(year(cast(visit_start_date as timestamp))) as visit_start_year
		  from visit_occurrence
...
from visit_occurrence vo1
   left join observation_period op1
	 on op1.person_id = vo1.person_id 
		and cast(vo1.visit_start_date as timestamp) >= cast(op1.observation_period_start_date as timestamp) 
		and cast(vo1.visit_start_date as timestamp) <= cast(op1.observation_period_end_date as timestamp)
...
from visit_occurrence vo1
where cast(visit_end_date as timestamp) < cast(visit_start_date as timestamp)
;
...
with rawData as
(
  select visit_concept_id as stratum_id, 
		datediff(cast(visit_end_date as timestamp), cast(visit_start_date as timestamp)) as count_value
  from visit_occurrence
),
overallStats as
(
...
select 220 as analysis_id,    
   cast(year(cast(visit_start_date as timestamp))*100 + month(cast(visit_start_date as timestamp)) as string) as stratum_1, 
   count(PERSON_ID) as count_value
from visit_occurrence
group by year(cast(visit_start_date as timestamp))*100 + month(cast(visit_start_date as timestamp))
;
