	...
	(
		...
		(
			...
			from @cdm_database_schema.concept c1
				inner join @cdm_database_schema.vocabulary v1 on c1.vocabulary_id = v1.vocabulary_id
		...
		from @cdm_database_schema.concept_ancestor ca0
		    ...
		    (select distinct c2.concept_id as os3_concept_id
		     from @cdm_database_schema.concept_ancestor ca1
				inner join @cdm_database_schema.concept c1 on ca1.DESCENDANT_CONCEPT_ID = c1.concept_id
			    inner join @cdm_database_schema.concept_ancestor ca2 on c1.concept_id = ca2.ANCESTOR_CONCEPT_ID
			    inner join @cdm_database_schema.concept c2 on ca2.DESCENDANT_CONCEPT_ID = c2.concept_id
	...
	(
		...
		from
		 (select DESCENDANT_CONCEPT_ID as os1_concept_id, concept_name as os1_concept_name
		  from @cdm_database_schema.concept_ancestor ca1
			  inner join @cdm_database_schema.concept c1 on ca1.DESCENDANT_CONCEPT_ID = c1.concept_id
		  where ancestor_concept_id = 4040390 and Min_LEVELS_OF_SEPARATION = 1
		 ) proc_by_os1
		 inner join
